...
namespace HIFUAcridTweaks.Misc
{
    internal class Passives : MiscBase
    {
        ...
        public override void Init()
        {
            ...
            passiveFamily.variants[0] = new SkillFamily.Variant
            {
                skillDef = regenerativeSD,
            };
            passiveFamily.variants[1] = new SkillFamily.Variant
            {
                skillDef = frenziedSD,
            ...
            regenerative.name = "HIFUAcridTweaks Regenerative";
            ...
            frenzied.name = "HIFUAcridTweaks Frenzied";
            ...
            frenSpeed = ConfigOption(0.25f, "Frenzied Movement Speed", "Decimal.");
        ...
        private void Bite_OnMeleeHitAuthority(ILContext il)
        {
            ...
            if (c.TryGotoNext(MoveType.Before,
                x => x.MatchCallOrCallvirt<CharacterBody>(nameof(CharacterBody.AddSpreadBloom))))
            {
                c.Index++;
                c.Emit(OpCodes.Ret);
            }
            ...
            {
                Main.HACTLogger.LogError("Failed to apply Regenerative Deletion 2 hook");
            }
        }
        ...
        private void Slash_OnMeleeHitAuthority(ILContext il)
        {
            ...
            {
                Main.HACTLogger.LogError("Failed to apply Regenerative Deletion 1 hook");
            }
        }
        ...
        private void Changes()
        {
            ...
            LanguageAPI.Add("KEYWORD_RAPID_REGEN", "<style=cKeywordName>Regenerative</style><style=cSub>Heal for <style=cIsHealing>" + Math.Round(regenHeal * 100, 2) + "%</style> of your maximum health over <style=cIsHealing>" + Math.Round(regenDur, 2) + "s</style>. <i>Can stack.</i></style>");
            LanguageAPI.Add("KEYWORD_RAPID_SPEED", "<style=cKeywordName>Frenzied</style><style=cSub>Gain <style=cIsUtility>" + Math.Round(frenSpeed * 100, 2) + "%</style> movement speed for <style=cIsUtility>" + Math.Round(frenDur, 2) + "s</style>. <i>Can stack.</i></style>");
        }
    }
